...
class fileHVACstaff implements file
{int count=0,k=0,c=0,l=0,z=0,y=0,kk=0;
Electricity e;HVAC h;AV a;Security sec;Housekeeping ho;supervisor sup;newuser n[];
	public fileHVACstaff(Electricity e,HVAC h,AV a,Security sec,Housekeeping ho,newuser n[])
	{
		this.e=e;this.h=h;this.sec=sec;this.a=a;this.ho=ho;this.n=n;
	}
	/**@brief reading file from the HVACstaff database
	 * @param i a counter
	 * @return void
	 */
	public void create(int i)
	{ try
	   { FileInputStream fin=new FileInputStream("HVACstaff.csv");  
	     count = 0; int ch=0;
	     char temp;
	     String str;
	     ...
	     { 
	    	...
	    	// System.out.println("sb: " + sb);
	    	 ...
	    	 {
	    		 h.arr[i].Setusername(str);count++;
	    	 
	    	 }
	    	 else if(count==2)
	    	 {h.arr[i].Setuserid(Integer.parseInt(str));count++;
	    	 }
	    	 else if(count==3)
	    	 {
	    	 
	    		 h.arr[i].Setpassword(str);count++;
	    	 ...
	    	 else if(count==4)
	    	 {
	    		 ...
	    		 h.arr[i].SetDOB(str);count++;
	    	
	    	 }
	    	 else if(count==5)
	    	 {
	    		 h.arr[i].Setaddress(str);count++;
	    	 ...
	    	 else if(count==6)
	    	 {
	    		 ...
	    		 h.arr[i].Setnot(Integer.parseInt(str));
	    		 if(h.arr[i].not==0)
	    	    	 count++;
	    	    	 c=h.arr[i].Getnot();
	    	    	 count++;
	    	 ...
	    	 l++;
	    	 h.arr[i].t[l].Setid(Integer.parseInt(str));
	    	...
	    	count++;
	    	 ...
	    	 else if(count==8)
	    	 {
	    		 ...
	    		 h.arr[i].l.Setfromdate(str);count++;
	    	 }
	    	 else if(count==9)
	    	 {
	    		 
	    		 h.arr[i].l.Settodate(str);count++;
	    		
	    	 }
	    	 else if(count==10)
	    	 {
	    		 
	    		 h.arr[i].l.Setreason(str);count++;
	    		
	    	 }
	    	 else if(count==11)
	    	 {
	    		 
	    		 h.arr[i].l.Setstatus(str);count++;
	    		
	    	 }
	    	 else if(count==12)
	    	 {
	    		 
	    		 h.arr[i].Setstatus(str.trim());
	...
	/**@brief for writing to the HVACstaff database
	...
	public void put(int ch,int ch2)
	{
		try
	 	{ 
	 		File inputFile = new File("HVACstaff.csv");
	 		FileWriter fw = new FileWriter("HVACstaff.csv",true);
	 	...
	 	for(k=1;k<=h.nos;k++)
	 	{
			...
			if(h.arr[k].l.Getfromdate().equals(Porsche)||h.arr[k].l.Gettodate().equals(Porsche))
				h.arr[k].Setstatus("on leave");
			else if(((str4.compareTo(str)<0)&&(str5.compareTo(str2)<0)&&(str6.compareTo(str3)<0)))
			{	h.arr[k].Setstatus("Available");}
	 	...
	 	fout.append(h.arr[k].Getname());
	 		
	 	fout.append(',');
	 	fout.append(h.arr[k].Getusername());
	 	fout.append(',');
	 	fout.append(String.valueOf(h.arr[k].Getuserid()));
	 	fout.append(',');
	 	fout.append(h.arr[k].Getpassword());
	 	fout.append(',');
	 	fout.append(h.arr[k].GetDOB());
	 	fout.append(',');
	 	fout.append(h.arr[k].Getaddress());
	 	fout.append(',');
	 	
	 	
	 	
	 	fout.append(String.valueOf(h.arr[k].not));
	 	fout.append(',');
	 	if(h.arr[k].not>0){
	 	for(j=1;j<h.arr[k].not;j++)
	 	{fout.append(String.valueOf(h.arr[k].t[j].Getid()));fout.append(',');}
	 	fout.append(String.valueOf(h.arr[k].t[h.arr[k].not].Getid()));
	 	fout.append(',');}
	 	
	 	
	 	fout.append(h.arr[k].l.Getfromdate());
	 	fout.append(',');
	 	fout.append(h.arr[k].l.Gettodate());
	 	fout.append(',');
	 	fout.write(h.arr[k].l.Getreason());
	 	fout.append(',');
	 	fout.write(h.arr[k].l.Getstatus());
	 	fout.append(',');
	 	fout.write(h.arr[k].Getstatus());
	...
	public void delete(int i)
	{
		try{
			...
			int k=0,id=0;
			File inputFile = new File("HVACstaff.csv");
			File tempFile = new File("HVACstaff1.csv");
			...
			wr.write("");
			wr.close();
			  InputStream in = new FileInputStream("HVACstaff1.csv");
			  OutputStream out = new FileOutputStream("HVACstaff.csv", true);
