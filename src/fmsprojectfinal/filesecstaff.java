...
class filesecstaff implements file
{int count=0,k=0,c=0,l=0,z=0,y=0,kk=0;
Electricity e;HVAC h;AV a;Security sec;Housekeeping ho;supervisor sup;newuser n[];
	public filesecstaff(Electricity e,HVAC h,AV a,Security sec,Housekeeping ho,newuser n[])
	{
		this.e=e;this.h=h;this.sec=sec;this.a=a;this.ho=ho;this.n=n;
	}
	/**@brief reading file from the Securitystaff database
	 * @param i a counter
	 * @return void
	 */
	public void create(int i)
	{ try
	   { FileInputStream fin=new FileInputStream("Securitystaff.csv");  
	     count = 0; int ch=0;
	     char temp;
	     String str;
	     ...
	     { 
	    	 ...
	    	 {
	    		 sec.arr[i].Setusername(str);count++;
	    	 
	    	 }
	    	 else if(count==2)
	    	 {sec.arr[i].Setuserid(Integer.parseInt(str));count++;
	    	 }
	    	 else if(count==3)
	    	 {
	    	 
	    		 sec.arr[i].Setpassword(str);count++;
	    	 ...
	    	 else if(count==4)
	    	 {
	    		 ...
	    		 sec.arr[i].SetDOB(str);count++;
	    	
	    	 }
	    	 else if(count==5)
	    	 {
	    		 sec.arr[i].Setaddress(str);count++;
	    	 ...
	    	 else if(count==6)
	    	 {
	    		 ...
	    		 sec.arr[i].Setnot(Integer.parseInt(str));
	    		 if(sec.arr[i].not==0)
	    	    	 count++;
	    	    	 c=sec.arr[i].Getnot();
	    	    	 count++;
	    	 ...
	    	 l++;
	    	 sec.arr[i].t[l].Setid(Integer.parseInt(str));
	    	...
	    	count++;
	    	 ...
	    	 { 
	    		 ...
	    		 sec.arr[i].l.Setfromdate(str);count++;
	    	 }
	    	 else if(count==9)
	    	 {
	    		
	    		 
	    		 sec.arr[i].l.Settodate(str);count++;
	    		
	    	 }
	    	 else if(count==10)
	    	 {
	    		 
	    		 sec.arr[i].l.Setreason(str);count++;
	    		
	    	 }
	    	 else if(count==11)
	    	 {
	    		 
	    		 sec.arr[i].l.Setstatus(str);count++;
	    		
	    	 }
	    	 else if(count==12)
	    	 {
	    		 
	    		 sec.arr[i].Setstatus(str.trim());
	...
	/**@brief for writing to the Securitystaff database
	...
	public void put(int ch,int ch2)
	{
		try
	 	{ 
	 		File inputFile = new File("Securitystaff.csv");
	 		FileWriter fw = new FileWriter("Securitystaff.csv",true);
	 	...
	 	for(k=1;k<=sec.nos;k++)
	 	{
	 		...
	 		if(ch==2&&ch2==sec.arr[k].Getuserid()){
			...
			sb.append(Porsche.charAt(7));
					...
					sb = new StringBuilder();
					String ss=sec.arr[k].l.Gettodate();
			...
			if(sec.arr[k].l.Getfromdate().equals(Porsche)||sec.arr[k].l.Gettodate().equals(Porsche))
				sec.arr[k].Setstatus("on leave");
			else if(((str4.compareTo(str)<0)&&(str5.compareTo(str2)<0)&&(str6.compareTo(str3)<0)))
			{	sec.arr[k].Setstatus("Available");}
	 	...
	 	fout.append(sec.arr[k].Getname());
	 		
	 	fout.append(',');
	 	fout.append(sec.arr[k].Getusername());
	 	fout.append(',');
	 	fout.append(String.valueOf(sec.arr[k].Getuserid()));
	 	fout.append(',');
	 	fout.append(sec.arr[k].Getpassword());
	 	fout.append(',');
	 	fout.append(sec.arr[k].GetDOB());
	 	fout.append(',');
	 	fout.append(sec.arr[k].Getaddress());
	 	fout.append(',');
	 	
	 	
	 	
	 	fout.append(String.valueOf(sec.arr[k].not));
	 	fout.append(',');
	 	if(sec.arr[k].not>0){
	 	for(j=1;j<sec.arr[k].not;j++)
	 	{fout.append(String.valueOf(sec.arr[k].t[j].Getid()));fout.append(',');}
	 	fout.append(String.valueOf(sec.arr[k].t[sec.arr[k].not].Getid()));
	 	fout.append(',');}
	 	
	 	
	 	fout.append(sec.arr[k].l.Getfromdate());
	 	fout.append(',');
	 	fout.append(sec.arr[k].l.Gettodate());
	 	fout.append(',');
	 	fout.write(sec.arr[k].l.Getreason());
	 	fout.append(',');
	 	fout.write(sec.arr[k].l.Getstatus());
	 	fout.append(',');
	 	fout.write(sec.arr[k].Getstatus().trim());
	 	...
	 	catch(Exception sec){System.out.println(sec);} 
	 	}
	...
	public void delete(int i)
	{
		try{
			...
			int k=0,id=0;
			File inputFile = new File("Securitystaff.csv");
			File tempFile = new File("Securitystaff1.csv");
			...
			wr.write("");
			wr.close();
			  InputStream in = new FileInputStream("Securitystaff1.csv");
			  OutputStream out = new FileOutputStream("Securitystaff.csv", true);
